...
namespace FTN.ESI.SIMES.CIM.CIMAdapter.Importer
{
	...
	public class PowerTransformerImporter
	{
			...
			if (cimSeasons != null)
			{
				foreach (KeyValuePair<string, object> cimSeasonPair in cimSeasons)
				{
					...
					if (rd != null)
					{
						...
						report.Report.Append("Terminal ID = ").Append(cimSeason.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("Terminal ID = ").Append(cimSeason.ID).AppendLine(" FAILED to be converted");
					}
				}
		...
		private void ImportRegControls()
		{
			...
			if (cimRegulatingControls != null)
			{
				foreach (KeyValuePair<string, object> cimRegulatingControlPair in cimRegulatingControls)
				{
					...
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
						report.Report.Append("PowerTransformer ID = ").Append(cimRegulatingControl.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("PowerTransformer ID = ").Append(cimRegulatingControl.ID).AppendLine(" FAILED to be converted");
					}
				}
				report.Report.AppendLine();
			}
		...
		private void ImportRegulationSchedules()
		{
			...
			if (cimRegulationSchedules != null)
			{
				foreach (KeyValuePair<string, object> cimRegulationSchedulePair in cimRegulationSchedules)
				{
					...
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
						report.Report.Append("TransformerWinding ID = ").Append(cimRegulationSchedule.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("TransformerWinding ID = ").Append(cimRegulationSchedule.ID).AppendLine(" FAILED to be converted");
					}
				}
				report.Report.AppendLine();
			}
		...
		private void ImportSynMachines()
		{
			...
			if (cimSynchronousMachines != null)
			{
				...
				{
					FTN.SynchronousMachine cimWindingTest = cimSynchronousMachinePair.Value as FTN.SynchronousMachine;

					ResourceDescription rd = CreateSynchronousMachineResourceDescription(cimWindingTest);
					if (rd != null)
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
